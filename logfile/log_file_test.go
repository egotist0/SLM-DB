...
func testOpenLogFile(t *testing.T, ioType IOType) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			gotLf, err := OpenLogFile(tc.args.path, tc.args.fid, tc.args.fSize, tc.args.fType, tc.args.ioType)
			...
			if (err != nil) != tc.wantErr {
				t.Errorf("OpenLogFile() error = %v, wantErr %v", err, tc.wantErr)
				return
			}
			if !tc.wantErr && gotLf == nil {
...
func testLogFileWrite(t *testing.T, ioType IOType) {
	...
	tests := []struct {
		...
		{
			"normal-1", fields{lf: lf}, args{buf: []byte("egotist")}, false,
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if err := tc.fields.lf.Write(tc.args.buf); (err != nil) != tc.wantErr {
...
func testLogFileRead(t *testing.T, ioType IOType) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, err := tc.fields.lf.Read(tc.args.offset, tc.args.size)
			if (err != nil) != tc.wantErr {
...
func testLogFileReadLogEntry(t *testing.T, ioType IOType) {
	...
	entries := []*LogEntry{
		...
		{Key: nil, Value: []byte("egotist"), ExpiredAt: 99400542343},
		{Key: []byte("k2"), Value: []byte("egotist"), ExpiredAt: 8847333912},
		{Key: []byte("k3"), Value: []byte("some data"), ExpiredAt: 8847333912, Type: TypeDelete},
	}
	var vals [][]byte
	for _, e := range entries {
		v, _ := EncodeEntry(e) // Write the LogEntry into
		vals = append(vals, v)
	}
	...
	tests := []struct {
		...
		{
			"read-entry-0", fields{lf: lf}, args{offset: offsets[4]}, &LogEntry{Key: []byte{}, Value: []byte("egotist"), ExpiredAt: 99400542343}, int64(len(vals[4])), false,
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, got1, err := tc.fields.lf.ReadLogEntry(tc.args.offset)
			if (err != nil) != tc.wantErr {
				t.Errorf("ReadLogEntry() error = %v, wantErr %v", err, tc.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tc.want) {
				t.Errorf("ReadLogEntry() got = %v, want %v", got, tc.want)
			}
			if got1 != tc.want1 {
				t.Errorf("ReadLogEntry() got1 = %v, want %v", got1, tc.want1)
			}
		})
	}
}
