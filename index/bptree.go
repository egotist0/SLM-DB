...
type BPTreeOptions struct {
	// DirPath path to store index meta.
	DirPath string
	// IndexType bptree(bolt)
	IndexType IndexerType
	// ColumnFamilyName db column family name, must be unique.
	ColumnFamilyName string
	// BucketName usually the same as column family name, must be unique.
	BucketName []byte
	// BatchSize flush batch size.
	BatchSize int
	// DiscardChn if values in indexer are changed,
	// the older value will be send to the DiscardChn.
...
func NewBPTree(opt BPTreeOptions) (*BPTree, error) {
	...
	// Begin starts a new transaction, writable set to true.
...
func (b *BPTree) Put(key, value []byte) error {
...
func (b *BPTree) PutBatch(nodes []*IndexerNode, opts WriteOptions) (offset int, err error) {
	...
	for batchIdx := 0; batchIdx < batchLoopNum; batchIdx++ {
		...
		bucket := tx.Bucket(b.opts.BucketName) //retrieves a bucket by name.
		...
		for itemIdx := offset; itemIdx < offset+b.opts.BatchSize; itemIdx++ {
			...
			if oldVal, err := bucket.Put(nodes[itemIdx].Key, meta); err != nil {
				_ = tx.Rollback() //Close the transaction and ignores all previous updates
				return offset, err
			} else if len(oldVal) > 0 && opts.SendDiscard { // Discard the old value.
				oldValues = append(oldValues, oldVal)
			}
		}
		if err := tx.Commit(); err != nil { // Writes all changes to disk and updates the meta page.
			return offset, err
		}
		if opts.SendDiscard {
...
// Close close bolt db.(releases all database resources)
